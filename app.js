const nextBtn = document.getElementById('next')
const prevBtn = document.getElementById('prev')
const slideImg = document.getElementById('slide-img')
const title = document.querySelector('.text-container h1')
const txt = document.querySelector('.text-container p')

const hamb = document.querySelector('.hamburger')
const nav = document.querySelector('.nav')



function toggle() {
    nav.classList.toggle('show-hamb')

    if(nav.classList.contains('show-hamb')){
        hamb.src = '/images/icon-close.svg'
    }else {
        hamb.src = '/images/icon-hamburger.svg'
    }
    
}


hamb.addEventListener('click', toggle)







const data = [
    {
        title: 'Discover innovative ways to decorate',
        text: `We provide unmatched quality, comfort, and style for property owners across the country. 
        Our experts combine form and function in bringing your vision to life. Create a room in your 
        own style with our collection and make your property a reflection of you and what you love.`,
        image: "/images/desktop-image-hero-1.jpg"
    },
    {
        title: 'We are available all across the globe',
        text: ` With stores all over the world, it's easy for you to find furniture for your home or place of business. 
        Locally, we’re in most major cities throughout the country. Find the branch nearest you using our 
        store locator. Any questions? Don't hesitate to contact us today.`,
        image: "/images/desktop-image-hero-2.jpg"
    },
    {
        title: 'Manufactured with the best materials',
        text: `Our modern furniture store provide a high level of quality. Our company has invested in advanced technology 
        to ensure that every product is made as perfect and as consistent as possible. With three decades of 
        experience in this industry, we understand what customers want for their home and office.`,
        image: `/images/desktop-image-hero-3.jpg`
    }
    
    
]
let counter = 0


function setNextData() {
    counter ++

    if(counter > 2) {
        counter = 0
    }
    slideImg.src = data[counter].image 
    title.innerText = data[counter].title
    txt.innerText = data[counter].text

}

function setPrevData() {
    
    counter--
        if(counter < 0) {
            counter = 2
        }
    slideImg.src = data[counter].image
    title.innerText = data[counter].title
    txt.innerText = data[counter].text
    
    
}



nextBtn.addEventListener('click', setNextData)
prevBtn.addEventListener('click', setPrevData)